...
def CheckTIR(arglist):
    ...
    for l in l_List:
        ...
        d = compare(s1, s2)
        if d < l*0.2 and Conserved(family, s1)==True:
                dic[str(rec.id)] = l
                break
    return dic
...
if __name__ == '__main__':
    ...
    for i in ["DTA","DTC","DTH","DTM","DTT"]:
       # cat="cat *%s > %s"%(i+"_RefLib-+-80.fa",genome_Name+spliter+i+spliter+"candidatesM2.fa")
        cat="for i in *%s; do cat $i; done > %s"%(i+"_RefLib-+-80.fa",genome_Name+spliter+i+spliter+"candidatesM2.fa") #shujun
        os.system(cat)
        rm ="rm *%s"%(i+"_RefLib-+-80")
        os.system(rm)
        rm ="rm *%s"%(i+"_RefLib-+-80.fa")
        os.system(rm)
    for i in ["DTA","DTC","DTH","DTM","DTT"]:
        file=genome_Name+spliter+i+spliter+"candidatesM2.fa"
